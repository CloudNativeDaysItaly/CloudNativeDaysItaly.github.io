...
const CommunityCard = ({ partner }) => {
    return (
        <div className="flex-grow-0 flex-shrink-0 basis-1/2 sm:basis-1/3 md:basis-1/4 lg:basis-1/5 p-3">
            <a href={partner.url} target="_blank" rel="noopener noreferrer" className="block group">
                <div className="flex items-center justify-center bg-white h-28 p-4 rounded-2xl border border-gray-200 shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                    <img
                        ...
                        style={{ maxHeight: '100%', maxWidth: '7rem' }}
                    />
                </div>
            </a>
        </div>
...
const CommunityTierSection = ({ tier, partners, config, uppercase = false, label, extraTopMargin = false }) => {
    ...
    return (
        <div className={`mb-16 ${extraTopMargin ? 'mt-20' : ''}`}>
            ...
            <h2 className={`text-2xl font-bold text-center text-gray-800 border-b pb-4 mb-6 ${uppercase ? 'uppercase' : 'capitalize'}`}>
                {config.title}
            </h2>
            {config.description && (
        ...
        for (const tier in communitiesByIds) {
            ...
            let partnerDetails = await Promise.all(
                partnerIds.map(async (partnerId) => {
                    const partnerPath = path.join(process.cwd(), 'src', 'config', 'communities', `${partnerId}.md`);
                    try {
                ...
                <div className="text-center max-w-3xl mx-auto">
                    ...
                    {Array.isArray(communityConfig.page.description)
                        ? communityConfig.page.description.map((p, i, arr) => (
                            i === arr.length - 1
                                ? <p key={i} className="mt-4 text-lg text-gray-600 font-semibold">{p}</p>
                                : <p key={i} className="mt-4 text-lg text-gray-600">{p}</p>
                          ))
                        : <p className="mt-4 text-lg text-gray-600">{communityConfig.page.description}</p>
                    }
                </div>
                ...
                <div className="mt-20">
                    {displayOrder.map((tier, i) => (
                        <CommunityTierSection
                            ...
                            label={
                                tier === 'community' ? 'Connect, live at the event!' :
                                tier === 'opensource' ? 'Discover, live at the event!' :
                                tier === 'media' ? 'Also supporting us' :
                                null
                            }
